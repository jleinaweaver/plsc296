...
**Why is the first version easier to read and what visualization lesson does that suggest?**

- Generally speaking, whichever variable has more levels should be your rows
...
```{r, echo=TRUE}
proportions(table(mpg$drv, mpg$trans))
```
...
These results are proportions, not probabilities, however we often hope the former tells us something about the latter

- A proportion summarizes how frequently some event actually happened in our sample

- A probability represents the chances of some event happening in the future and is typically estimated using the proportion

<br>

The key to this connection comes down to our confidence that the sample we are studying actually represents the full population

- **Put your research design hats on and tell me how confident you are in treating our sample as representative of all cars on the road?**
...
Once again, and for always, without understanding the methodology of our measures we really can't say anything about the wider world with our data!

- HOWEVER, once you know the method you can interpret the data, even if the data isn't perfect!
...
**Previously, what function did we use to calculate the descriptive statistics for a nueric variable like city fuel economy?**
...
- **Summarize city fuel economy in this dataset for me**

<br>

**SLIDE**: We'll use the aggregate function to calculate the bivariate version of descriptive statistics
:::
...
::: notes
Clearly, aggregate will work with two numeric variables but the output won't tell you what you think it does
...
## Cut: Convert Numeric to Factor {background-image="libs/Images/background-slate_v2.png" .center .smaller}

<br>

```{r, echo=TRUE}
# Use the cut function and save the new variable in mpg
mpg$displ2 <- cut(x = mpg$displ, breaks = c(0, 4, 8), labels = c("Small", "Large"))
```
...
::: {.fragment}
Redo our exercise using the quartiles of `displ`
...
**REVEAL**: Let's practice this again.
...
## For next class analyze the relationship between: {background-image="libs/Images/background-slate_v2.png" .center}
