...
- Low score is "good", high score is "bad"
...
- Pairs / Small groups: Build an OVERALL index of CT belief that includes all eight questions, summarize the results and report back your findings
...
**SLIDE**: Let's now shift our focus to the political knowledge questions
...
- **How can we accomplish this in R? Ideas?**

<br>

My advice:

- You'll need to convert each of these variables into a numeric dummy with a "1" if the respondent gave the correct answer and a "0" otherwise

- Save these new variables and the index will be much simpler to make later!
...
d |>
  select(Majority_House2:PM_Canada2) |>
  pivot_longer(cols = Majority_House2:PM_Canada2, names_to = "question", values_to = "values") |>
  group_by(question) |>
  summarize(
    N = n(),
    Yes = sum(values, na.rm=T),
    Proportion = Yes / N
  ) |>
  arrange(desc(Proportion)) |>
  kable(digits = 2) |>
...
1. Build and evaluate the political knowledge index,

2. Read p81-98 in Chapter 4

3. Complete Exercises 2 and 4
...
1. Construct an additive index of political knowledge using the Miller, Saunders and Farhart (2016) data. Submit a polished visualization of your constructed index AND your analysis of the results.
