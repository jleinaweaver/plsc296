...
Today we begin exploring regressions

- A powerful and incredibly common tool frequently used by scientists of all stripes for estimating relationships

<br>

My assumption: You have completed AND reflected on the readings for today

- This discussion should NOT be your intro to regression!

- As we move deeper into stats you will have to encounter concepts repeatedly for them to make sense!
...
- The diamonds dataset includes details on 53k+ diamondsin terms of their characteristics and estimated prices
...
Excellent, now let's get more specific!

<br>

It's one thing to note an association between two variables (e.g. bigger diamonds are typically more expensive)

- What we need is a numerical estimate for the relationship

- In other words, as we move up in diamond weight, how big a jump in price should we expect to see?
...
diamonds |>
  ...
  theme_bw() +
  labs(x = "Carats", y = "Price",
       title = "How expensive is a move from 1.5 to 2.5 carats?") +
  scale_y_continuous(labels = scales::dollar_format()) +
  coord_cartesian(xlim = c(0,4)) +
...
For example, here I am showing you the range of prices at two sizes in the dataset: 1.5 and 2.5 carats
...
## Using Descriptive Statistics {background-image="libs/Images/background-slate_v2.png" .center .smaller}

<br>

:::: {.columns}
::: {.column width="40%"}
```{r, fig.align = 'left', fig.asp=.9, fig.width = 6, cache=TRUE}
range1 <- range(diamonds$price[diamonds$carat == 1.5])
range2 <- range(diamonds$price[diamonds$carat == 2.5])
...
**1. Choosing a statistic is somewhat arbitrary**
...
::: notes
The FIRST big problem with this approach to answering our question is that it is somewhat arbitrary
...
This is why descriptive stats are important for helping the reader see your data but not necessarily for supporting a strong argument on its own.
...
**2. No measures of uncertainty (beyond the sd for the mean)**

```{r, echo=TRUE}
mean(diamonds1_5$price)
sd(diamonds1_5$price)
```

<br>

```{r, echo=TRUE}
mean(diamonds2_5$price)
sd(diamonds2_5$price)
```
...
Regression can help us with all three issues!

- Simple OLS regression is not arbitrary as it assumes most natural processes exhibit a "regression to the mean",

- It produces its estimates based on ALL of the data, AND

- Its estimates come with measures of uncertainty!

<br>

**SLIDE**: So, we need a tool like regression that has these characteristics, but why should "regression to the mean" be a good baseline assumption?
:::
...
## Fitting OLS Regressions in R {background-image="libs/Images/background-slate_v2.png" .center .smaller}
...
**SLIDE**: Let's untangle these results
:::



## An OLS Estimate of Diamond Prices {background-image="libs/Images/background-slate_v2.png" .center .smaller}
...
::: notes
The estimates here in the middle is the main element we've been working with today
...
::: notes
For reporting OLS results in a report
...
**SLIDE**: Last bit of new code, automated model predictions!
:::


## Making Predictions {background-image="libs/Images/background-slate_v2.png" .center .smaller}
