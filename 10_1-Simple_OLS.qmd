...
## Why 'Regression'? {background-image="libs/Images/background-slate_v2.png" .center}
...
:::: {.columns}
::: {.column width="50%"}
```{r, fig.asp=0.9, fig.align = 'center', fig.width=7, cache=TRUE}
diamonds |>
  #slice_sample(prop = .4) |>
  ggplot(aes(x = carat)) +
  geom_histogram(bins = 25, color = "white") +
  ...
  annotate("text", x = 3, y = 12500, label = str_c("Mean(Carats) = ", round(mean(diamonds$carat), 2)), size = 6)
```
:::
...
diamonds |>
  ...
  scale_x_continuous(labels = scales::dollar_format()) +
  annotate("text", x = 12500, y = 12500, label = str_c("Mean(Price) = ", round(mean(diamonds$price), 2)), size = 6)
```
:::
::::
...
:::: {.columns}
::: {.column width="50%"}
```{r, fig.asp=0.9, fig.align = 'center', cache=TRUE}
diamonds |>
  #slice_sample(prop = .4) |>
  ggplot(aes(x = carat)) +
  geom_histogram(bins = 25, color = "white") +
...
## What is the predicted price of a 1.5 carat diamond? {background-image="libs/Images/background-slate_v2.png" .center}
...
## What is the predicted price of a 2.5 carat diamond? {background-image="libs/Images/background-slate_v2.png" .center}
...
## Predicted price difference of a 1.5 and a 2.5 carat diamond? {background-image="libs/Images/background-slate_v2.png" .center}
...
```{r, fig.asp=0.618, fig.align = 'center', fig.width=9, cache=TRUE}
library(fivethirtyeight)

bechdel2 <- bechdel |>
  mutate(
    box_office = (domgross_2013 + intgross_2013)/1e9,
    budget = budget_2013/1e6
  )

bechdel_labels <- bechdel2 |>
  filter(box_office > 2 | budget > 270)
...
ggplot(data = bechdel2, aes(x = budget, y = box_office)) +
  ...
  theme_bw() +
  labs(x = "Budget (2013 USD)", y = "Box Office (2013 USD)", title = "Analyzing Movie Box Office Returns", caption = "Source: FiveThirtyEight") +
...
## What explains the variation in movie box office? {background-image="libs/Images/background-slate_v2.png" .center}
...
## An OLS Estimate of Diamond Prices {.center .smaller}
