...
**Which one of these three measures do you think is the hardest to accurately measure?**
...
res1 <- tibble(x1 = rep(NA_integer_, 100))

for (i in 1:100) {
  
  res1$x1[i] <- sum(sample(x = c(1, 0), size = 10, 
                           replace = TRUE, prob = c(.8,.2)))
  
}

# Visualize the results
hist(res1$x1, main = "100 Surveys of 10 Students Each")
```
...
- Some of our surveys estimated the "yes" percentage as low as 50% and others as high as 100%
...
for (i in 1:100) {
  ...
  res1$x1[i] <- sum(sample(x = c(1, 0), size = 100, 
                           replace = TRUE, prob = c(.8,.2)))
  
}

# Visualize the results
hist(res1$x1, main = "100 Surveys of 100 Students Each")
```
...
- (Ten student survey gave us a wide range from 50% to 100%)

- (100 students has shrunk that down to 70-90%)
...
for (i in 1:100) {
  ...
  res1$x1[i] <- sum(sample(x = c(1, 0), size = 1000, 
                           replace = TRUE, prob = c(.8,.2)))
  
}

# Visualize the results
hist(res1$x1, main = "100 Surveys of 1,000 Students Each")
```
...
for (i in 1:100) {
  ...
  res1$x1[i] <- sum(sample(x = c(1, 0), size = 10000, 
                           replace = TRUE, prob = c(.8,.2)))
  
}

# Visualize the results
hist(res1$x1, main = "100 Surveys of 10,000 Students Each")
```
...
The complexity comes from properly interpreting the CI

- Too often people assume the 95% CI tells us the range of values that contain the true population parameter

- ACTUALLY, the 95% CI tells us the range of values that contain the true population parameter in 95% of repeated samples
...
In slightly simpler terms, your estimated 95% CI has 95% chance of including the true population parameter.
...
**SLIDE**: Let's review your assignments for today
...
- And the z-scores are how many SEs the true parameter could be
...
A 90% CI replaces the 1.96 with 1.64

- So you give a range of 1.64 x SE above and below your estimate
