...
```{r, echo=TRUE, fig.align='center', fig.width=6, fig.asp=.75, cache=TRUE, `code-line-numbers`="3,4"}
ggplot(mpg, aes(x = class, fill = drv)) +
  geom_bar(position = "fill") +
...
::: notes
Since the Y-axis is now proportions we can label the values as percentages with scale_y_continuous.

- scale_y_continuous lets you tweak the y-axis

- In this case I've kept all the defaults but asked it to change the labeling of each tick mark

<br>

I've also removed the y axis label because these are proportions and not counts
:::
...
```{r, echo=TRUE, fig.align='center', fig.width=6, fig.asp=.75, cache=TRUE, `code-line-numbers`="5"}
ggplot(mpg, aes(x = class, fill = drv)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "", y = "") +
...
::: notes
And since we have added color to the bars using the "fill" argument, we can adjust those colors using the scale_fill_manual function
...
```{r, echo=TRUE, `code-line-numbers`="4", fig.align='center', cache=TRUE}
ggplot(mpg, aes(x = cty)) +
  geom_histogram(bins = 15) +
  theme_bw() +
  facet_wrap(~ drv)
...
::: notes
One option is to use a facet_wrap
...
**SLIDE**: Not the easiest thing to compare in this way
:::


## {background-image="libs/Images/background-slate_v2.png" .center .smaller}

```{r, echo=TRUE, `code-line-numbers`="4", fig.align='center', cache=TRUE, fig.asp=1.3, fig.width=6}
ggplot(mpg, aes(x = cty)) +
  geom_histogram(bins = 15) +
  theme_bw() +
...
::: notes
Use the "ncol" argument to specify you want a single column of plots
...
**SLIDE**: Facet wraps are handy, but this is a problem box plots were designed to solve!
:::
