...
- Today's class SHOULD NOT be your intro to OLS regression!

- In fact, this should be your THIRD journey into OLS counting the Wheelan reading, the reading for today and our work in class on Monday
...
**What's the smallest RSS anybody in class came up with?**
...
**SLIDE**: Why go to all this trouble?
...
::: {.r-fit-text}
**How are changes in X associated with changes in Y?**
:::
...
- On average...
    ...
    - When X equals zero, Y equals 1.4
    
    - Each one unit increase in X is associated with a 0.5 increase in Y
    
    - When X equals 10, Y equals 6.4

::: notes

Keep in mind, regression analysis is fundamentally like all the rest of statistics

- A tool meant to summarize data.

<br>

So, rather than showing someone a scatter plot of 50,000 data points you can communicate that relationship using this formula for a line

- This simple line summarizes all of that data and let's us answer the question with specificity

- Every one unit increase in X is associated with a .5 increase in Y

<br>

This could be:

- measures of government spending and economic growth

- time spent studying and scores on the LSAT

- OR an answer to our class research question!

<br>

The regression line is the next big step in you learning how to answer substantial and important questions with data.

- **SLIDE**: It turns out that in addition to giving us specific answers to our questions, regression analysis offers other improvements on our bivariate analyses
...
First things first, I want to reinforce the usefulness of regression analysis by highlighting these three points

- I want you coming away from our work convinced that regression is a powerful tool for doing science that you WANT to better understand.

<br>

**SLIDE**: Let's start with the first one
...
- When trying to represent a relationship why not use the mean since the world works so hard to push us to the average?
...
**SLIDE**: For the second and third reason scientists use regressions will take us back to our diamonds data
...
## Using Descriptive Statistics {background-image="Images/background-data_blue_v4.png" .center}
...
## The Limits of Descriptive Statistics {background-image="Images/background-data_blue_v4.png" .center}

<br>

::: {.r-fit-text}
**1. Choosing a summary point to compare is arbitrary**
:::
...
::: {.r-fit-text}
**2. It is hard to describe our uncertainty**
:::
...
#   labs(x = "Carats", y = "Price") +
#   scale_y_continuous(labels = scales::dollar_format()) +
#   annotate("text", x = .5, y = 27500, hjust = 0, label = paste0("Price = ", round(coef(res1)[[1]], 2), " + ", round(coef(res1)[[2]], 2), " X"), size = 5)
# 
...
As we worked on last class, regression is a technique for estimating the relationship between predictor variables (X) and an outcome (Y) using the formula for a line.

- It usefully summarizes the relationship using a simple formula that we can use to make predictions

- AND it generates this line while accounting for ALL of the observations in the dataset!
...
- *Show them the alpha and beta but leave the rest for later*

<br>

Remind me how I interpret this formula

- **What does an alpha of -2256 mean?** 

    - (A 0 carat diamond is worth negative dollars!)

- **What does a beta of 7,756 mean?** 

    - (On average, each one carat increase in diamond size is associated with a price increase of nearly $8k!)
    
<br>

Let's use this line to answer our question about 1.5 vs 2.5 carat diamonds
...
We'll get to confidence intervals down the road, but I want you to see that this should always be provided when you make a prediction with an OLS regression.
...
Alright, let's build this line of best fit

- **According to the readings, how does OLS find the line of best fit?**
...
You can think of this like a process in which the computer repeatedly draws a line, calculates the RSS, and then repeats its action until it finds a line with the lowest RSS

- Technically, it actually uses matrix algebra to calculate the OLS line...

<br>

**SLIDE**: summarize the "why" we use regression
...
**Any questions on these intuitions?**

<br>

Let's now talk about the four steps I want you to use to evaluate the fit of a regression line to the data
...
- Survey Bias (Social-desirability and Non-response)
...
- If your regression is being fit on less than ALL the data in your sample it's up to you to make an argument that the missing data isn't a serious problem for your analyses

<br>

If your missing data is truly random, then it isn't as big a problem
...
## Evaluating the Fit of an OLS Regression {background-image="Images/background-data_blue_v4.png" .center}
