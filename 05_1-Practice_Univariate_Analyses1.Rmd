...
# Install a package of extra datasets
#install.packages("carData")

# Load the new package to access the UN dataset (2009–2011)
library(carData)

# 1. What is the regional breakdown of UN member states? (Bar plot)
practice1 <- ggplot(data = UN, aes(x = region))
practice1 + geom_bar()

# 2. How much variation is there in life expectancy rates around the world?  (Compare a histogram to a boxplot)
practice2 <- ggplot(data = UN, aes(x = lifeExpF))
practice2 + geom_histogram()
practice2 + geom_boxplot()

# 3. Visualize the growth in the US population since 1790 (line plot)
